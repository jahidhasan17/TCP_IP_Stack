...
static unsigned int udp_port_number = 2000;
...
int send_pkt_out(char *pkt, unsigned int pkt_size, interface_t *interface) {
    ...
    if (!nbr_node) {
        printf("Nrb node not found to send data\n");
        return -1;
    }
...
static void *_network_start_pkt_receiver_thread(void *arg) {
    ...
    ITERATE_GLTHREAD_BEGIN(&topo->node_list, curr) {
        ...
        printf("Node : %s, UDP Port is: %d, and Socket FD is : %d\n", node->node_name, node->udp_port_number, node->udp_sock_fd);
